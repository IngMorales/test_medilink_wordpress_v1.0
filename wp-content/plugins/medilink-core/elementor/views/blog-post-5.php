						...
						<?php if ( $data['meta']  == 'yes' ): ?>							
							<?php if ( RDTheme::$options['blog_date'] ): ?>
								<div class="post-date add-pimg">
									<i class="fa fa-calendar" aria-hidden="true"></i> <?php the_time( get_option( 'date_format' ) );?>
									<span> &nbsp;</span>
								<i class="fa fa-user" aria-hidden="true"></i>  <?php esc_html__( 'by' , 'medilink-core' ); ?>  <span class="vcard author"><?php the_author_posts_link();?></span>
							</div>
							<?php endif; ?>		
						<?php endif ?>
						<?php } else{ ?>
