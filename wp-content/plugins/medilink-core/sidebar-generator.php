...
class Sidebar_Generator {
	...
	public $medilink = MEDILINK_THEME_PREFIX_VAR;
	public $option_name = null;

	public function __construct() {
		$this->option_name = $this->medilink . '_custom_sidebars';
		...
		<div class="widgets-holder-wrap">
			<div id="rdtheme-new-sidebar" class="widgets-sortables">
				<div class="sidebar-name">
					...
					<h2><?php esc_html_e( 'Add New Sidebar', 'medilink-core' ); ?><span class="spinner"></span></h2>
				</div>
				<div class="sidebar-description">
					<form method="POST" action="<?php echo esc_attr( admin_url( 'admin-ajax.php?action=rdtheme_add_sidebar' ) );?>">
						<?php wp_nonce_field( 'rdtheme_add_sidebar' ); ?>
						<table class="form-table">
							<tr>
								<th scope="row"><?php esc_html_e( 'Name', 'medilink-core' ) ?></th>
								<td><input type="text" class="text" name="name" value=""></td>
								<td><input type="submit" class="button-primary" value="<?php esc_html_e( 'Add', 'medilink-core' ) ?>"></td>
							</tr>
						</table>
					</form>
				</div>
	...
	public function load_scripts() {
		...
		wp_enqueue_script( 'admin-sidebar-generator', Helper::get_js( 'admin-sidebar-generator' ), array( 'jquery' ), MEDILINK_THEME_VERSION );

		$localize_data = array(
			'confirm'  => esc_html__( 'Are you sure you want to remove this custom sidebar', 'medilink-core' ),
			'failed'   => esc_html__( 'Operation failed' , 'medilink-core' ),
	...
	public function ajax_add_sidebar() {
		...
		if ( empty( $name ) ) {
			wp_send_json_error( esc_html__( "Sidebar name can't be empty", 'medilink-core' ) );
		}

		if ( empty( $nonce ) ) {
			wp_send_json_error( esc_html__( 'Empty nonce', 'medilink-core' ) );
		}

		if ( ! wp_verify_nonce( $nonce, 'rdtheme_add_sidebar' ) ) {
			wp_send_json_error( esc_html__( 'Invalid nonce', 'medilink-core' ) );
		}
		...
		if ( array_key_exists( $id, $sidebars ) ) {
			wp_send_json_error( esc_html__( 'Sidebar with the same name already exists. Please choose a different name', 'medilink-core' ) );
		}
	...
	public function ajax_remove_sidebar() {
		...
		if ( empty( $id ) ) {
			wp_send_json_error( esc_html__( 'Sidebar ID not found', 'medilink-core' ) );
		}
