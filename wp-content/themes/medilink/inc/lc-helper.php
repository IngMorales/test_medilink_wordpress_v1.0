...
class Helper {
    ...
    public function __construct() {
        ...
        $medilink = $theme_info->get('Name'); 

        // theme name
        $this->medilink = $medilink; 

        // theme slug
        $medilink = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $medilink)));
        $this->theme_slug = $medilink;  
    }
    ...
    public function theme_menu() { 
        add_theme_page( esc_html__( 'Theme License', 'medilink' ), esc_html__( 'Theme License', 'medilink' ), "manage_options", "rtlc", array( $this, 'create_admin_page' ), null, 99);
    }
    ...
    public function create_admin_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Theme License', 'medilink' ); ?></h1>
            <form method="post" action="options.php">
            <?php
                // This prints out all hidden setting fields
                settings_fields( 'rt_option_group' );
                do_settings_sections( 'medilink-setting' ); 
            ?>
            </form>
        </div>
        <?php
    ...
    public function theme_option() {        
        ...
        add_settings_section(
            ...
            false, // Title
            false, // Callback
            'medilink-setting' // Page
        );  

        add_settings_field(
            'rt_purchase_code',  
            esc_html__( 'Purchase Code', 'medilink' ),
        ...
        add_settings_field(
            'rt_license_status', 
            esc_html__( 'License Status', 'medilink' ), 
    ...
    public function license_status_callback() {  
        ...
        $status_text = esc_html__( 'Not Activated', 'medilink' );
         
        if ( rtlc_is_valid()['success'] ) {
            $verify = true;
            $status_text = esc_html__( 'Activated', 'medilink' ); 
        } else if ( isset( rtlc_is_valid()['domain_match'] ) && !rtlc_is_valid()['domain_match'] ) {  
            $status_text = esc_html__( 'Domain Mismtach', 'medilink' ); 
        }
    ...
    public function license_note_callback() { 
        $status = esc_html__( 'Please keep in mind, you can activate one license in one domain, if you face any problem in activation, please contact our', 'medilink' ).' <a href="https://www.radiustheme.com/contact/">'.esc_html__( 'Support Center', 'medilink' ).'</a>'; 
    ...
    public function license_check_callback() {
        printf(
            '<input type="button" id="rtlc_license_check" class="button button-primary rtcl-active-btn" value="%s" /> <span class="rtlc-loader"><i class="dashicons dashicons-update spin"></i></span>', esc_html__( 'Activate License', 'medilink' )
        );
    }
    ...
    function rtlc_verification() {   
        ...
        if ( $purchase_code ) { 
            ...
            $medilink = $this->medilink;   
            $domian_name = esc_url( home_url() );  

            $api_url = "{$rt_license_server}/?theme_name={$medilink}&purchase_code=" . $purchase_code . "&domain_name=" . $domian_name;   
